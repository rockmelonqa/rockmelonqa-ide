...
class AfterRunHandlerDotnet implements IAfterRunHandler {
  ...
  async processRecordings(context: IRunTestContext, logBuilder: StringBuilder) {
    ...
    if (recordings.length) {
      ...
      await this.copyRecordingsToTestRuns(context, recordings);
    }
  }
  ...
  /** Copies error screenshot that are generated by the run to the `test-runs/screnshots` folder */
  ...
  /** Copies recordings that are generated by the run to the `test-runs/recordings` folder */
  private async copyRecordingsToTestRuns(context: IRunTestContext, failedTestScreenshots: string[]) {
    const dotnetOutputFolder = path.join(
      context.rmProjFile.folderPath,
      StandardFolder.OutputCode,
      "bin",
    ...
    for (let file of failedTestScreenshots) {
      let filePath = path.join(dotnetOutputFolder, file);
      let copyToPath = path.join(recordingFolder, file);
      await fs.copyFile(filePath, copyToPath);
      await fs.unlink(filePath);
    }
  }
