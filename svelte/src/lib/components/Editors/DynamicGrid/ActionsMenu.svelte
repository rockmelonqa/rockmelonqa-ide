...
<div class="h-full flex gap-2 flex-nowrap items-center justify-center px-2" data-row-index={index}>
    ...
    {/each}
    {#if buttons.length > visibleButtons}
        <div class="dropdown relative" on:focusout={handleDropdownFocusLoss}>
            <IconLinkButton on:click={handleDropdownClick}>
                <svelte:fragment slot="icon">
                    <EllipsisIcon />
                </svelte:fragment>
            </IconLinkButton>
